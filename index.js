...
function prevItem() {
  currentIndex =
    (currentIndex - 1 + carouselItemsImg.length) % carouselItemsImg.length;
  showItem(currentIndex);
}
...
prevButton2.addEventListener("click", () => {
  itemWidth = carouselItemsText[0].offsetWidth; // Recalcula o itemWidth atualizado
  currentIndex2 =
    (currentIndex2 - 1 + carouselItemsText.length) % carouselItemsText.length;
  carouselContainer.style.transform = `translateX(-${
    currentIndex2 * itemWidth
  }px)`;
});
